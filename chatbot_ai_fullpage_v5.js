...
#widget .voice-btn{ 
  padding: 16px; border: 1px solid rgba(43, 43, 43, 0.15); background: transparent; 
  color: var(--muted); display: grid; place-items: center; cursor: pointer; 
  border-radius: 10px; transition: all .15s ease;
}
#widget .voice-btn:hover{ background: rgba(43, 43, 43, 0.05); color: var(--ink); }
#widget .voice-btn.recording{ 
  background: #ff4444; color: white; border-color: #ff4444; 
  animation: pulse 1.5s infinite;
}
#widget .voice-btn svg{ width: 18px; height: 18px; stroke: currentColor; }
@keyframes pulse{
...
/* Clear Memory Button */
#widget .clear-memory-container{
  display: flex; justify-content: flex-end; padding: 0 20px 10px; margin-bottom: -10px;
}
#widget .clear-memory-btn{
  display: flex; align-items: center; gap: 8px; padding: 8px 16px; border: 1px solid var(--line);
  background: transparent; color: var(--muted); font-size: 14px; font-weight: 500;
  border-radius: 8px; cursor: pointer; transition: all .15s ease; font-family: var(--font);
}
#widget .clear-memory-btn:hover{
  background: rgba(43, 43, 43, 0.05); color: var(--ink); border-color: var(--ink);
}
#widget .clear-memory-btn svg{
...
<section class="widget" id="widget" role="dialog" aria-modal="true" aria-label="Chat and contact">
  ...
  <div class="page active" id="page-chat">
    ...
    <div class="clear-memory-container">
      <button class="clear-memory-btn" id="clearMemory" aria-label="Clear Chat Memory">
    ...
    <div class="composer">
      ...
      <button class="voice-btn" id="voiceBtn" aria-label="Voice Input" title="Click to speak">
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" viewBox="0 0 24 24">
...
`;
        ...
        window.debugState = () => {
          console.log('🔍 Current conversation state:', conversationState);
          console.log('🔍 Registration form used:', registrationFormUsed);
        ...
        function handleSend() {
          ...
          if (conversationState.waitingForName) {
            console.log('>>> BRANCH: waitingForName');
            // User provided their name
            conversationState.userName = text;
            ...
            console.log('>>> BRANCH: waitingForQuery');
            // User provided their query or browsing response
            const lowerText = text.toLowerCase();
            
            if (lowerText.includes('just browsing') || lowerText.includes('not sure') || 
                lowerText.includes('browsing') || lowerText.includes('not sure')) {
              
              conversationState.waitingForQuery = false;
              ...
              setTimeout(() => {
                removeTyping();
                addMsg(`Got it, no pressure. I can share a few ways we usually help partners, or we can just chat and see where it takes us. What feels better for you?`, "bot");
              }, 1000);
              
            } else {
              // User has a specific query - proceed to Discovery (call API)
              conversationState.waitingForQuery = false;
              
              console.log('>>> SUB-BRANCH: specific query - calling API');
              ...
              const apiTimeout = setTimeout(() => {
                if (!responseSent) {
                  ...
                  removeTyping();
                  console.log('API call timed out, using fallback response');
                  addMsg("I'm doing well, thank you! How can I assist you today?", "bot");
                  responseSent = true;
                }
              ...
              callChatAPI(text)
                .then(response => {
                  if (!responseSent) {
                    ...
                    // Parse API response - API returns 'response' field
                    const botResponse = response.response || response.message || response.text || "I'm sorry, I couldn't process your request.";
              ...
              // Guaranteed response mechanism - ensure user always gets a response
              setTimeout(() => {
                if (!responseSent) {
                  console.log('🔄 Guaranteed response: ensuring user gets a response');
                  removeTyping();
                  addMsg("I'm doing well, thank you! How can I assist you today?", "bot");
                  responseSent = true;
                }
              }, 3000); // 3 second guaranteed response
            }
          } else {
            console.log('>>> BRANCH: normal conversation flow');
            // Normal conversation flow - call API
            console.log('Normal conversation flow - processing:', text);
            ...
            const apiTimeout = setTimeout(() => {
              if (!responseSent) {
                console.log('⏰ Normal flow API call timed out!');
                removeTyping();
                console.log('Normal flow API call timed out, using fallback response');
                addMsg("I'm doing well, thank you! How can I assist you today?", "bot");
                responseSent = true;
              }
            }, 4000); // 4 second timeout
            ...
            callChatAPI(text)
              .then(response => {
                if (!responseSent) {
                  clearTimeout(apiTimeout);
                  removeTyping();
                  console.log('✅ Normal flow API response:', response);
                  // Parse API response - API returns 'response' field
                  const botResponse = response.response || response.message || response.text || "I'm sorry, I couldn't process your request.";
                  console.log('📝 Normal flow bot response to display:', botResponse);
                  addMsg(botResponse, "bot");
                  console.log('✅ Normal flow message added to chat');
                  responseSent = true;
                }
              })
              .catch(error => {
                if (!responseSent) {
                  clearTimeout(apiTimeout);
                  removeTyping();
                  console.error('❌ Normal flow API error:', error);
                  console.log('🔄 Normal flow using fallback response');
                  addMsg("I'm doing well, thank you! How can I assist you today?", "bot");
                  console.log('✅ Normal flow fallback message added to chat');
                  responseSent = true;
                }
              });
              
            // Guaranteed response for normal flow too
            setTimeout(() => {
              if (!responseSent) {
                console.log('🔄 Guaranteed response (normal flow): ensuring user gets a response');
                removeTyping();
                addMsg("I'm doing well, thank you! How can I assist you today?", "bot");
                responseSent = true;
              }
          ...
          voiceBtn.addEventListener("click", function() {
            if (!isRecording) {
              try {
                ...
                addMsg("Failed to start voice input. Please try again.", "bot");
              }
            } else {
              recognition.stop();
            }
        ...
        if (saveUserInfo) {
          saveUserInfo.addEventListener("click", () => {
            ...
            // Update conversation state to allow user to continue conversation
            conversationState.userName = username;
            conversationState.waitingForName = false;
            conversationState.waitingForQuery = true;
            
            console.log('Form submission - Updated conversation state:', conversationState);
